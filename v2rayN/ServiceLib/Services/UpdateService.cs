...
public class UpdateService(Config config, Func<bool, string, Task> updateFunc)
{
    ...
    {
        var customUrl = _config?.ConstItem.GeoSourceUrl;
        ...
        foreach (var geoName in files)
        {
            ...
            {
                urls.Add(string.Format(customUrl, geoName));
            }
            foreach (var backupUrl in Global.GeoUrlBackups)
            {
                urls.Add(string.Format(backupUrl, geoName));
            }

            await DownloadGeoFileWithFallback(urls, fileName, targetPath);
        }
    }
    ...
    {
        var customUrl = _config.ConstItem.SrsSourceUrl;
        ...
        if (!string.IsNullOrEmpty(customUrl))
        {
            urls.Add(string.Format(customUrl, type, $"{type}-{srsName}", srsName));
        }
        foreach (var backupUrl in Global.SingboxRulesetUrlBackups)
        {
            urls.Add(string.Format(backupUrl, type, $"{type}-{srsName}", srsName));
        }
    ...
    {
        await DownloadGeoFileWithFallback(new List<string> { url }, fileName, targetPath);
    }

    private async Task DownloadGeoFileWithFallback(List<string> urls, string fileName, string targetPath)
    {
        ...
        foreach (var url in urls)
        {
            ...
            try
            {
                await UpdateFunc(false, $"Trying: {url}");
                ...
                downloadHandle.UpdateCompleted += (sender2, args) =>
                {
                    ...
                    if (!args.Success)
                    {
                        _ = UpdateFunc(false, $"Failed: {args.Msg}");
                    }
                };
                downloadHandle.Error += (sender2, args) =>
                {
                    tcs.TrySetResult(false);
                    _ = UpdateFunc(false, $"Error: {args.GetException().Message}");
                };

                await downloadHandle.DownloadFileAsync(url, tmpFileName, true, _timeout);

                if (await tcs.Task && File.Exists(tmpFileName))
                {
                    File.Copy(tmpFileName, targetPath, true);
                    File.Delete(tmpFileName);
                    await UpdateFunc(false, string.Format(ResUI.MsgDownloadGeoFileSuccessfully, fileName));
                    success = true;
                    break;
                }
            }
            ...
            {
                await UpdateFunc(false, $"Exception downloading from {url}: {ex.Message}");
            }
        }

        if (!success)
        {
            await UpdateFunc(false, $"All sources failed for {fileName}");
        }
    }
